...
use backend\modules\bills\models\Bgtb;
use backend\modules\bills\models\Bills;
use backend\modules\bills\models\SearchBgtb;
...
/**
 * BgtbdienController implements the CRUD actions for Bgtbdien model.
 */
class BgtbController extends Controller
{
    /**
     * @inheritdoc
     */
  public function behaviors()
    ...
    /**
     * Lists all Bgtbdien models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new SearchBgtb();
    ...
    /**
     * Displays a single Bgtbdien model.
     * @param integer $id
     * @return mixed
     */
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            foreach(bgtb::find()->where(['key'=>$id])->all() as $row){
...
//        var_dump($this->findBModel($id)->devicekey);exit;
//               $dataProvider = new ActiveDataProvider([
//            'query' =>  $this->findBModel($id),
//            'pagination' => [
//        		'pageSize' => 15 // in case you want a default pagesize
//        	],'sort'=>[ 'defaultOrder' => [
//            'id' => SORT_DESC,
...
$mpdf->WriteHTML($this->renderAjax('printbarcode', [
            'model' => $this->findBModel($id)->devicekey ,
        ]));
...
public function actions()
   {
        return ArrayHelper::merge(parent::actions(), [
           'edititems' => [                                       // identifier for your editable column action
               ...
               'modelClass' => Bgtb::className(),                // the model for the record being edited
...
//                'postOnly' => true,
    ...
    /**
     * Deletes an existing Bgtbdien model.
    ...
    /**
     * Finds the Bgtbdien model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Bgtbdien the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Bgtb::findOne(['key'=>$id])) !== null) {
            return $model;
        } else {
